/**
 * The navigation model, defined once so the sidebar and the bottom bar can't
...
export type NavItem = {
  ...
  label: string;
  /**
   * Bottom-bar glyph, above the label. Placeholder geometry - pick something
   * that means anything at all for your own destinations. The sidebar is
   * deliberately text-only, so this is the only place it shows.
   */
  icon: string;
};
